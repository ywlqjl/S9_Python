...
def cal():
    ...
    myButton = partial(Button, root, padx=10, pady=3)
    ...
    button0.grid(row=4, column=0, columnspan=2, padx=5, pady=5, sticky=N+S+E+W)

    button13 = myButton(text='.', command=lambda : get_input(entry, '.'))
    button13.grid(row=4, column=2, pady=5)

    button14 = Button(root, text='/', padx=10, pady=3, command=lambda : get_input(entry, '/'))
    button14.grid(row=4, column=3, pady=5)

    button15 = Button(root, text='<-', padx=10, pady=3, command=lambda : backspace(entry))
    button15.grid(row=5, column=0, pady=5)

    button16 = Button(root, text='C', padx=10, pady=3, command=lambda : clear(entry))
    button16.grid(row=5, column=1, pady=5)

    button17 = Button(root, text='=', padx=10, pady=3, command=lambda : calc(entry))
...
#def calTan(entry):
    ...
    # try:
    # result2 = math.tan(int((input.strip()* math.pi)/180))
